...
/*   Updated: 2023/06/23 15:51:05 by ctardy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	last_calcul(t_game *game, t_calc *n, t_data img)
{
	...
	// tex_pos = ((double)(*n).se_draw[0] - ((double)(*n).h / 2)) + ((*n).line_height / 2) * step;
	...
	while ((*n).se_draw[0] < (*n).se_draw[1])
	{
		...
		// printf("Valeur de draw 0 : %d\n", (*n).se_draw[0]);
		// printf("Valeur de draw 1 : %d\n", (*n).se_draw[1]);
		(*n).se_draw[0]++;
	}
}
